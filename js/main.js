...
if(!users[user]){
users[user]={followers:0,following:0,likes:0,avatar:""};
}
...
function vote(i){
theories[i].votes++;
if(users[theories[i].author]){
users[theories[i].author].likes++;
}
save();render("theoryFeed");render("featuredContainer");
}
function comment(i,text){
if(!text)return;
theories[i].comments.push({user:current||"Anonymous",text});
save();render("theoryFeed");render("featuredContainer");
}
...
return `What if ${s} ${t}? Evidence suggests hidden anomalies across human history.`;
}
